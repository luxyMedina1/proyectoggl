...
**`npm run lint` sale rojo y es esperado.** 158 errores + 146 warnings heredados del código legacy
(de los errores, 124 son `@typescript-eslint/no-explicit-any`: se van tipando por lotes y por
carpeta, no de una — ver `utils/apiError.ts` y `types/Conferencia.ts`, tipos compartidos que ya
mataron ~30). Se muestra sin bloquear; cuando llegue a 0, hay que quitar el `continue-on-error` de
los dos CI para que empiece a gatear. No lo "arregles" con un `--fix` masivo dentro de un PR de otra
cosa.
...
Los números de este archivo (34 páginas, 23 cliente, 111 `<img>`, 158 errores de lint, 135 pruebas,

...
import { useEventosStore, SIN_CACHE_DISPONIBILIDAD } from "../../../../hooks/useEventosStore";
import type { CabeceraEvento } from "./cabeceraEvento";
...
// `'use client'`): el Server Component `page.tsx` invoca `proyectarCabeceraEvento` al resolver el
// evento, y una función exportada desde este módulo de cliente no puede llamarse desde el servidor.
// Se re-exporta el tipo para no romper los imports existentes que lo traían desde aquí.
export type { CabeceraEvento };

interface DetalleEventoProps {
  cabecera?: CabeceraEvento;
}
...
function DetalleEventoContent({ cabecera }: DetalleEventoProps) {
      ...
      if (id) {
        ...
        try {
          ...
          if (isAbono) {
            ...
            const eventoRes = await getDetalleEventos(eventoId.toString(), SIN_CACHE_DISPONIBILIDAD);
            ...
            if (funcionId && response.funciones) {
              ...
              if (funcion) {
                ...
                response.aperturaPuertas =
                  funcion.aperturaPuertas ?? response.aperturaPuertas ?? null;
  ...
  const funcionSecciones: string | number | null = isAbono
    ? ((
        (modoAbono === "por_funcion" ? funciones[currentFuncionIndex] : funciones[0]) ??
        funciones[0]
      )?.id ?? null)
  ...
  useEffect(() => {
    ...
    if (destino === `${pathname}${searchParams.toString() ? `?${searchParams.toString()}` : ""}`)
      return;
  ...
  useEffect(() => {
    const fetchSecciones = async () => {
      if (id && !esperandoFuncionSecciones) {
        try {
          // secciones[].asientosDisponibles es la disponibilidad que ve el comprador:
          // siempre fresca, nunca de caché (Req 26.3, anti venta doble).
          const response = await getDetalleEventoSecciones(
            id,
            funcionSecciones,
            SIN_CACHE_DISPONIBILIDAD,
          );
  ...
  // Efecto sobre el estado de compra al elegir una sección. Es el ÚNICO punto donde vive la
  // lógica de selección: tanto el clic sobre el mapa (`handleSectionClick`) como la lista
  // accesible por teclado (`SeccionesAccesibles`) la invocan con la MISMA sección, de modo que
  // el efecto es idéntico por construcción (Req 16.3, Property 10). No toca el DOM del SVG:
  // el resaltado visual del mapa se hace aparte en `handleSectionClick`.
  const seleccionarSeccion = (seccion: Secciones) => {
    if (seccion.tipo_seccion != "general") {
      if (isAbono) {
        const fId = funcionSecciones ?? id;
        const aId = searchParams.get("abonoId") || id;
        router.push(
          `/abonos/${slug}/${seccion.id}/${seccion.nombre}?funcionId=${fId}&abonoId=${aId}`,
        );
      } else {
        // La funcion ya viaja dentro del slug del evento.
        router.push(
          `/eventos/${evento ? buildEventoSlug(evento, funcionActiva) : slug}/${seccion.id}/${seccion.nombre}`,
        );
      }
    } else {
  ...
  useEffect(() => {
    ...
    sections.forEach((section, index) => {
      const seccionData = secciones.find(
        (s: Secciones) =>
      ...
      if (seccionData) {
        ...
        const seccionOculta = seccionesOcultas?.find(
          (s: Secciones) =>
            s.nombre.toLowerCase() === section.id.toLowerCase() &&
            s.bloque.toLowerCase() ===
              (section.attributes.getNamedItem("bloque")?.value.toLowerCase() || ""),
    ...
    <div>
        ...
        <div className="container mx-auto px-4 md:px-5 lg:px-8 2xl:px-20">
          ...
          <div className="mb-3">
            <h2 className="text-2xl font-semibold text-gray-800 flex items-center gap-x-5">
              ...
              <p className="whitespace-nowrap text-center">
                ...
                {!evento?.recinto?.esGeneral && (
                  <span className="block font-normal text-base text-gray-500">
                    Para empezar haz clic en el mapa
                  </span>
                )}
              </p>
              <div className="border border-gray-400 grow rounded-full"></div>
            ...
            {(seccionesAdicionales ?? []).length > 0 && eventoActivo && (
              <>
                <p className="text-gray-700 text-xl font-semibold my-4">
                  Boletos Generales Adicionales{" "}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {seccionesAdicionales?.map((seccion, index) => (
                    <div
                      ...
                      className="border border-gray-300 rounded-md bg-white shadow-sm p-2 lg:p-3 flex flex-col items-start justify-between"
                    >
                      <h5 className="text-base font-light text-gray-700 flex flex-wrap items-center gap-x-1">
                        <span
                          className="rounded-md w-5 h-5 grid place-items-center"
                          style={{ backgroundColor: seccion.colorGeneral }}
                        >
                          <TbTicket className="text-white" size={16} />
                        </span>
                      ...
                      <button
                        onClick={() => {
                          handleClickSeccionAdicional(seccion);
                        }}
                        disabled={
          ...
          {isAbono && (
            <div className="mb-8 p-4 bg-white rounded-xl shadow-sm border border-gray-100">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-gray-500 text-sm">
                    ¿Deseas el mismo asiento para todas las fechas o elegir por separado?
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-sm text-gray-600">
                    Mantener asiento en todas las fechas
                  </span>
                  <button
                    onClick={() => {
                      const nuevoModo =
                        modoAbono === "por_funcion" ? "mismo_asiento" : "por_funcion";
              ...
              <div className="flex justify-center mb-3">
                <p className="font-normal text-sm text-slate-500 text-center inline-flex mx-auto border border-slate-300 rounded-full px-4 py-3">
                  {funciones.length} fechas incluidas en el pase
                </p>
              </div>
              ...
              {modoAbono === "por_funcion" && funciones.length > 0 && (
                <div className="flex justify-center gap-3 overflow-x-auto py-2 scrollbar-none">
                  {funciones.map((f, index) => {
                    ...
                    const isLocked =
                      index > 0 &&
                      !seleccionesAbono.some((s) => s.funcionId === funciones[index - 1].id);
                    ...
                    return (
                      ...
                      >
                        <span className="text-[10px] font-bold uppercase tracking-wider mb-1">
                          {f.fecha ? formatDate(f.fecha, "MMM yyyy") : ""}
                        </span>
                        <span className="text-xl font-black mb-1">
                          {f.fecha ? formatDate(f.fecha, "dd") : ""}
                        </span>
                        <span className="text-[10px] font-medium">
                          {f.fecha ? formatDate(f.fecha, "EEEE") : ""}
                        </span>
                      </button>
                    );
                  })}
              ...
              {((getAbonoBuilderState()?.seleccionesParciales?.length || 0) > 0 ||
                Object.keys(getAbonoBuilderState()?.asientosPorFecha || {}).length > 0) && (
                <button
                  onClick={() => {
                    clearAbonoBuilderState();
                    setSeleccionesAbono([]);
          ...
          <div
            ref={containerRef}
            className="relative container-svg my-5 bg-white shadow-sm p-2 lg:p-3 rounded-lg"
          >
            {eventoActivo ? (
              <>
                {renderSVG()}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-40">
          <div
            className={`bg-white p-2 md:p-3 rounded-lg shadow-lg min-w-96 ${step === 2 ? "w-8/12" : ""} max-h-[95vh] overflow-y-auto`}
          >
            <button
              ...
              <>
                <div className="text-emphasis font-semibold text-xl flex flex-wrap items-center justify-between mb-5">
                  {modalProps?.asientosDisponibles ? (
                    <p className="capitalize">
                      {modalProps?.nombreEspecial}, {modalProps?.bloque} -{" "}
                      {modalProps?.tipo_seccion}{" "}
                    </p>
                  ) : (
                    <p className="capitalize">
                  ...
                  <p className="text-sm font-normal">
                    Disponibles:
                    <span className="px-1 rounded-full shadow-sm bg-blue-100 text-xs font-semibold">
                      {modalProps?.asientosDisponibles}
                    </span>
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2">
                  <div className="col-span-2 md:col-span-1 flex items-center gap-x-2">
                    ...
                    <span className="text-emphasis capitalize">
                ...
                <div className="flex items-center justify-between mb-4">
                  <label htmlFor="boletos_general" className="text-gray-500 font-medium">
                    Cantidad de boletos
                  </label>
                  <div className="flex items-center border border-gray-300 px-2 py-1 rounded-full">
                    ...
                    <input
                      ...
                      type="number"
                      value={boletos}
                      className="p-2 text-center w-16 rounded-md"
                    ...
                    <button
                      ...
                      className="w-8 h-8 bg-accentBase rounded-full grid place-items-center"
                    >
                ...
                <div className="grid gap-y-5">
                  <button
                    ...
                    className={`px-4 py-2 bg-accentBase hover:bg-emphasis transition-colors text-neutral rounded-md ${modalProps?.asientosDisponibles === null || modalProps?.asientosDisponibles === 0 ? "cursor-not-allowed" : ""}`}
                  >
                    Siguiente
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 text-gray-500">
                  <section className="col-span-2 lg:col-span-1">
                    <div className="grid grid-cols-3 gap-3 mb-3">
                      <div className="flex flex-col items-start gap-y-2 col-span-3 lg:col-span-2">
                        <p className="bg-gray-100 rounded-full px-3 py-1 inline-block text-sm text-gray-600">
                          {boletos} {boletos === 1 ? "asiento" : "asientos"}{" "}
                          {boletos === 1 ? "seleccionado" : "seleccionados"}
                        </p>
                        <p className="text-sm">
                          Evento <span className="block font-medium">{evento?.nombre}</span>
                        </p>
                        <p className="text-sm">
                          Fecha{" "}
                      ...
                      <div className="flex flex-col gap-y-2 col-span-3 lg:col-span-1">
                        ...
                        <p className="text-right text-sm">
                          Recinto{" "}
                          <span className="block font-medium">
                            {evento?.recinto?.nombre}, {evento?.ciudad?.nombre}
                          </span>
                        </p>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 font-medium flex justify-between px-2 lg:px-3">
                        Boleto <span>Precio</span>
                      </p>
                      <ul className="grid gap-1 overflow-y-auto max-h-32 pr-1">
                        {Array.from({ length: boletos }).map((_, index) => (
                          <li
                            key={index}
                            className="bg-gray-100 rounded-md p-2 text-sm text-gray-600 flex items-center gap-1"
                          >
                    ...
                    <div>
                      ...
                      <div className="px-2 lg:px-3">
                        <p className="flex items-center justify-between text-sm">
                          {boletos} x {boletos === 1 ? "boleto" : "boletos"}:{" "}
                          <span className="font-medium">
                            ${Math.floor(boletos * precioBoletos * 100) / 100}
                          </span>
                        </p>
                        {discountAmount > 0 && (
                          <p className="flex items-center justify-between text-sm">
                        ...
                        <p className="flex items-center justify-between text-sm">
                          Subtotal:{" "}
                          <span className="font-medium">
                            ${Math.floor((boletos * precioBoletos - discountAmount) * 100) / 100}
                          </span>
                        </p>
                        <p className="flex items-center justify-between text-sm">
                          Cargo por servicio:{" "}
                          <span className="font-medium">
                            $
                            {Math.floor(uds * (boletos * precioBoletos - discountAmount) * 100) /
                              100}
                          </span>
                        </p>
                        <p className="flex items-center justify-between text-sm">
                        ...
                        {ivaApplied && (
                          <p className="flex items-center justify-between text-sm">
                            ...
                            <span className="font-medium">
                              $
                              {Math.floor(iva * (boletos * precioBoletos - discountAmount) * 100) /
                                100}
                            </span>
                          </p>
                        )}
                        <hr className="my-2" />
                        <p className="flex items-center justify-between text-emphasis font-bold text-xl lg:text-2xl">
                  ...
                  <section className="col-span-2 lg:col-span-1 flex flex-col gap-1 relative">
                          ...
                          <div className="grid grid-cols-8 divide-y gap-4">
                            <div className="col-span-8 md:col-span-8">
                              ...
                              <figure className="flex flex-wrap gap-3 items-center mt-2">
                                <Image
                                  className="aspect-3/2 object-contain w-6"
                                  width={60}
                                  height={40}
                                  src="/visa.png"
                                  alt=""
                      ...
                      {tabMetodo === "nueva_tarjeta" && !evento?.esGratuito && (
                        <>
                          <div className="space-y-1">
                            <label
                              htmlFor="nombre"
                              className="text-sm text-gray-400 font-semibold pb-1 block"
                            >
                              Nombre del titular
                            </label>
                            <input
                    ...
                    <footer className="mbs-auto">
                      <p className="text-center w-full text-xs my-3 text-gray-400">
                        Al hacer clic en el botón pagar, confirmas que has leído y aceptas nuestros
                      ...
                      {reservaExitosa && (
                        <>
                          <button
                            ...
                            type="button"
                            className="w-full px-4 py-2 bg-accentBase hover:bg-emphasis mb-2 transition-colors text-white rounded-lg"
                          >
                            Pagar
                          </button>
                          <button
                            ...
                            className="w-full px-4 py-2 bg-gray-300 hover:bg-gray-400 transition-colors text-gray-700 rounded-lg"
                          >
                            Cancelar compra
                          </button>
                        </>
        ...
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-50">
          <div className={`bg-white p-2 md:p-3 rounded-lg shadow-lg min-w-96 max-w-96 text-center`}>
            <div className="rounded-full w-12 h-12 bg-green-500 grid place-items-center mx-auto mb-5">
              <LuBadgeCheck className="text-white text-3xl" />
            </div>
            ...
            {!usuarioInvitado && (
              <Link
                ...
                className="w-full block px-4 py-2 bg-accentBase hover:bg-emphasis mb-2 transition-colors text-white rounded-lg"
              >
                Ir a mis compras
              </Link>
            )}

...
function SeccionAsientoContent() {
  ...
  const { getDetalleEventos, getFilasSeccion, reservar, cancelar, resolverSlugEvento } =
    useEventosStore();
  ...
  // NOTA (TDZ / React Compiler): las siguientes funciones se declaran aquí, por encima de
  // sus primeros usos en `procesarPago` y en los efectos de abajo. Es una reubicación pura
  ...
  const validarDescuento = async (num_asientos: any[]) => {
    ...
    try {
      ...
      if (res.data.id) {
        ...
        const asientosPorCategoriaPrecio = agruparAsientosPorCategoriaPrecio(
      ...
      <div className={`grid gap-4 ${reservaExitosa ? "lg:grid-cols-1" : "lg:grid-cols-2"}`}>
        ...
        <div>
          ...
          <h3 className="text-lg font-semibold text-gray-600 mb-2 mt-3 text-left">
            Asientos seleccionados: <span>{asientosSeleccionados?.length}</span>
          </h3>
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-x-2">
                    <span
                      className="grid place-items-center rounded-md w-8 h-8"
                ...
                <p className="text-gray-600 font-normal text-sm">
                  Asiento:{" "}
                  <span className="font-bold text-gray-800">{asiento.fila + asiento.numero}</span>
                </p>
              </div>
            ))}
          </div>

...
export default function TerminarCompra() {
  return (
    <Suspense fallback={<LocalLoader />}>
      <TerminarCompraContent />
    </Suspense>
  );
}

function TerminarCompraContent() {
  const { reservaId, esGeneral, promocionId } = useParams<{ reservaId: string; esGeneral: string; promocionId: string }>();

...
const AbonoSeccionAsientoContent = () => {
  ...
  const { slug, seccionId, seccion } = useParams<{
    slug: string;
    seccionId: string;
    seccion: string;
  }>();
  ...
  const [cargosPorCategoria, setCargosPorCategoria] = useState<
    { id: number; categoria: string; cargoPorCategoria: string }[]
  >([]);
  ...
  const [cantidadesPromocion, setCantidadesPromocion] = useState<{
      ...
      if (eventoId && seccionId) {
        ...
        const funcionId = (
          modoSeleccion === "por_funcion"
            ? (funciones[currentFuncionIndex] ?? funciones[0])
            : funciones[0]
        ).id.toString();
  ...
  useEffect(() => {
    if (promocion_id != 0) {
      ...
      const asientosAgrupados = agruparAsientosPorCategoriaPrecio(
      ...
      if (!aplicaTodoEvento) {
        ...
        if (categoriasAplicables.length === 0) {
          toast.error(
            `Promocion solo valida para las siguientes categorías: ${categoriasValidas.join(", ")}`,
          );
          setDiscountAmount(0);
          setDiscountPorcent(0);
          setPromocionID(0);
      ...
      if (tipoPromocion === "CANTIDAD") {
        ...
        const { descuentoTotal: descuentoAplicado, algunaCategoriaCumple } =
          calcularDescuentoCantidadPorAsientos(
            asientosSeleccionados as unknown as AsientoPromo[],
            cantidadCompra,
            cantidadPaga,
        ...
        if (!algunaCategoriaCumple) {
          toast.error(
            `Debes seleccionar al menos ${cantidadCompra} asientos en una misma categoría y precio para aplicar este código.`,
          );
          setDiscountAmount(0);
          setDiscountPorcent(0);
          setPromocionID(0);
    ...
    try {
      ...
      if (res.data.id) {
        ...
        if (!promo.aplicaTodoEvento) {
          ...
          if (categoriasAplicables.length === 0) {
            toast.error(
              `Promocion solo valida para las siguientes categorías: ${[...categoriasGenerales, ...categoriasValidas].join(", ")}`,
            );
            setDiscountAmount(0);
            setDiscountPorcent(0);
            setPromocionID(0);
        ...
        if (res.data.tipo == "CANTIDAD") {
          ...
          const { descuentoTotal: totalDescuento, algunaCategoriaCumple } =
            calcularDescuentoCantidadPorAsientos(
              asientosSeleccionados as unknown as AsientoPromo[],
              cantidadCompra,
              cantidadPaga,
  ...
  useEffect(() => {
    if (discountCode == "") {
      const categoriasSeleccionadas = [
        ...new Set(asientosSeleccionados.map((asiento) => asiento.categoria)),
      ];
      const promosValidas = filtrarPromocionesAplicablesPorCategorias(
        promocionesAplicanDirecto,
        categoriasSeleccionadas,
      );
      const mejorPromocion = obtenerMejorPromocionPorAsientos(
        promosValidas,
        asientosSeleccionados as unknown as AsientoPromo[],
        subtotalesPorCategoria,
      );
      ...
      if (mejorPromocion) {
        ...
        setCategoriasPromo(
          mejorPromocion.categorias as unknown as { categoria: { nombre: string } }[],
        );
        ...
        // Recalcular el cargo por servicio (UDS) sobre los subtotales YA descontados por
        ...
        const descuentoPorCat = calcularDescuentoPorCategoriaDeAsientos(
          {
            ...
            porcentaje: mejorPromocion.porcentaje,
            cantidadResta: mejorPromocion.cantidadResta,
            cantidadCompra: mejorPromocion.cantidadCompra,
            cantidadPaga: mejorPromocion.cantidadPaga,
            aplicaTodoEvento: mejorPromocion.aplicaTodoEvento,
          },
          asientosSeleccionados as unknown as AsientoPromo[],
          subtotalesPorCategoria,
    ...
    return (
      <div className="p-3 grid place-items-center">
        ...
        <div className="flex items-center gap-x-3 text-3xl">
          <span className="font-semibold text-red-500 w-12 h-12 p-1 rounded shadow grid place-items-center">
            {minutos.toString().padStart(2, "0")}
          </span>
          <span className="font-semibold w-12 h-12 p-1 rounded shadow grid place-items-center">
            :
          </span>
          <span className="font-semibold text-red-500 w-12 h-12 p-1 rounded shadow grid place-items-center">
  ...
  const handleAsientosNoDisponibles = (
    ...
    if (modoSeleccion === "mismo_asiento") {
      funciones.forEach((f) => {
        const noDispFunc = noDisponibles
          .filter((nd) => String(nd.funcionId) === String(f.id))
          .map((nd) => Number(nd.asientoId));
      ...
      seleccionesPrevias.forEach((sel) => {
        ...
        const falló = noDisponibles.some(
          (nd) =>
    ...
    if (currentFuncId) {
      ...
      const nuevosSeleccionados = asientosSeleccionados.filter((a) =>
        disponiblesAhora.includes(a.id),
      );
  ...
  const handleReservarAsientos = async (e: { preventDefault: () => void }) => {
    ...
    try {
      if (eventoId) {
        ...
        const payloadAbono: ReservarAbonoBody = {
          modo: modoSeleccion,
        };
        ...
        if (user) {
          ...
          if (response && response.completo === false && response.noDisponibles) {
            return handleAsientosNoDisponibles(
              response.noDisponibles,
              idsAsientos,
              seleccionesFormateadas,
            );
          }
          ...
          if (response && response.reservaId) {
            ...
            try {
              ...
              // propio -> la excepción escapaba sin controlar y el abono (ya
        ...
        <div className="bg-[#e6e6e8] overflow-auto container-seats rounded-lg">
          {filas.map((fila) => (
            <div key={fila.id} className="p-1">
              <div
                className={`flex items-center justify-${orientacion == "left" ? "start" : orientacion == "right" ? "end" : "center"} gap-2`}
              >
                <span className="w-8 flex-none h-8 bg-white border flex items-center justify-center shadow-sm text-sm square-full">
                  {fila.nombre}
                </span>
                {fila.asientos.map((asiento: Asiento) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={
                        !isMesa
                          ? () => handleAsientoClick({ ...asiento, fila: fila.nombre })
                          : undefined
                      }
                      onMouseEnter={() =>
                        asiento.estado !== "inaccesible"
                          ? setHoveredAsiento({ ...asiento, fila: fila.nombre })
                          : null
                      }
                    ...
                    >
                      {estaSeleccionado ? (
                        ...
                        >
                          <g clipPath="url(#clip0_3372_85057)">
                            <g clipPath="url(#clip1_3372_85057)">
                              <path
                                d="M3.5 18C4.33 18 5 17.33 5 16.5V15H15V16.5C15 17.33 15.67 18 16.5 18C17.33 18 18 17.33 18 16.5V14C18 12.9 17.1 12 16 12H4C2.9 12 2 12.9 2 14V16.5C2 17.33 2.67 18 3.5 18ZM18 7H19C19.55 7 20 7.45 20 8V9C20 9.55 19.55 10 19 10H18C17.45 10 17 9.55 17 9V8C17 7.45 17.45 7 18 7ZM1 7H2C2.55 7 3 7.45 3 8V9C3 9.55 2.55 10 2 10H1C0.45 10 0 9.55 0 9V8C0 7.45 0.45 7 1 7ZM15 12H5V2C5 0.9 5.9 0 7 0H13C14.1 0 15 0.9 15 2V12Z"
                                fill="#EF4444"
                              />
                              <g filter="url(#filter0_d_3372_85057)">
                                <path
                                  d="M7.25 5.75C7.25 5.02065 7.53973 4.32118 8.05546 3.80546C8.57118 3.28973 9.27065 3 10 3C10.7293 3 11.4288 3.28973 11.9445 3.80546C12.4603 4.32118 12.75 5.02065 12.75 5.75C12.75 6.47935 12.4603 7.17882 11.9445 7.69454C11.4288 8.21027 10.7293 8.5 10 8.5C9.27065 8.5 8.57118 8.21027 8.05546 7.69454C7.53973 7.17882 7.25 6.47935 7.25 5.75ZM5.5 11.5C5.5 10.837 5.76339 10.2011 6.23223 9.73223C6.70107 9.26339 7.33696 9 8 9H12C12.663 9 13.2989 9.26339 13.7678 9.73223C14.2366 10.2011 14.5 10.837 14.5 11.5V13H5.5V11.5Z"
                                  fill="#F4F4F5"
                                />
                              </g>
                            </g>
                          ...
                          <defs>
                            ...
                            >
                              ...
                              <feBlend
                                ...
                                result="effect1_dropShadow_3372_85057"
                              />
                              <feBlend
                                mode="normal"
                                in="SourceGraphic"
                                in2="effect1_dropShadow_3372_85057"
                                result="shape"
                              />
                            </filter>
                        ...
                        <svg
                          ...
                          xmlns="http://www.w3.org/2000/svg"
                          style={{
                            fill: asiento.estado === "disponible" ? asiento.color : "currentColor",
                          }}
                        >
    ...
    return (
      <form className="space-y-4 mt-5">
        <div className="grid grid-cols-8 divide-y-2 md:divide-y-0 md:divide-x-2 gap-4">
          <div className="col-span-8 md:col-span-3">
            ...
            <figure className="flex flex-wrap gap-3 items-center mt-3">
              <img
                className="aspect-3/2 object-contain w-10"
                width={60}
                height={40}
                src="/visa.png"
                alt=""
              ...
              />
              <img
                className="aspect-3/2 object-contain w-20"
                width={60}
                height={40}
                src="/carnet.png"
                alt=""
        ...
        <div className="space-y-1">
          <label htmlFor="nombre" className="text-sm text-gray-600 font-semibold pb-1 block">
            Nombre del titular
          </label>
          <input
            type="text"
        ...
        <div className="space-y-1">
          <label htmlFor="tarjeta" className="text-sm text-gray-600 font-semibold pb-1 block">
        ...
        <div className="flex space-x-2">
          <div className="space-y-1 w-full">
            <label htmlFor="expiracion" className="text-sm text-gray-600 font-semibold pb-1 block">
          ...
          <div className="space-y-1 w-full">
            <label htmlFor="cvv" className="text-sm text-gray-600 font-semibold pb-1 block">
              Código de seguridad{" "}
              <Tooltip tooltip="Código de seguridad de 3 a 4 dígitos al reverso de tu tarjeta.">
          ...
          {tarjetas.length > 0 && (
            <div>
              ...
              {tarjetas.map((tarjeta) => (
                <div
                  className="flex items-center gap-x-3 bg-gray-100 relative hover:bg-gray-200 transition-colors rounded-lg p-2 mb-2 group"
    ...
    return (
      <div className={`grid gap-4 ${reservaExitosa ? "lg:grid-cols-1" : "lg:grid-cols-2"}`}>
        ...
        <div>
          ...
          <h3 className="text-lg font-semibold text-gray-800 mb-2 mt-3 text-left">
            Resumen de tu Abono
          </h3>

          {/* Fechas del Abono */}
          <div className="mb-4 bg-slate-50 border border-slate-200 rounded-lg p-3 shadow-sm">
            <p className="text-sm font-medium text-slate-700 mb-2 border-b border-slate-200 pb-1">
              Fechas incluidas:
            </p>
              ...
              >
                ...
                <div className="flex items-center justify-between w-full mt-3">
                  <div className="flex items-center gap-x-3">
                    <span
                      className="grid place-items-center rounded-lg w-10 h-10 shadow-sm"
                      style={{ backgroundColor: asiento.color || "#000000" }}
                    >
                    ...
                    <div className="flex flex-col">
                      ...
                      <p className="text-gray-500 font-medium text-xs mt-0.5">
                        Asiento:{" "}
                        <span className="font-bold text-gray-900 text-sm bg-gray-100 px-1.5 py-0.5 rounded">
        ...
        <div className={`${reservaExitosa ? "lg:order-last" : "lg:order-first"}`}>
          ...
          */}
            ...
            <div className="col-span-2 text-xs pl-0 flex gap-2 items-center justify-between my-2 text-emphasis font-semibold">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="w-36 p-2 border border-gray-400 bg-white rounded-md text-gray-900 text-right placeholder:text-gray-600"
                  style={{ textTransform: "uppercase" }}
                />
                <button
                  type="button"
                  className="bg-accentBase text-neutral px-2 rounded-md transition-colors"
          ...
          <div>
            ...
            {discountAmount > 0 && (
              <p className="flex items-center justify-between text-emphasis text-lg font-semibold">
                Descuento {promoNombre} :{" "}
                <span className="text-emphasis text-lg font-semibold">-${discountAmount}</span>
              </p>
            )}
            ...
            <div className="flex justify-between mt-4 w-full items-center">
              <span className="text-base text-emphasis font-semibold">Cargo por servicio:</span>
            ...
            {iva !== null && iva !== undefined && !isNaN(iva) && iva > 0 && (
              <div className="flex justify-between mt-1 w-full items-center">
                <span className="text-base text-emphasis  font-semibold">
                  IVA
                  <Tooltip tooltip="Impuesto sobre el valor añadido">
                    <IoInformationCircle className="text-gray-500 text-lg" />
                  </Tooltip>
                </span>
                <span className="text-emphasis  text-base font-semibold">
                ...
                {modoSeleccion === "por_funcion" ? (
                  <button
                    onClick={() => {
                      ...
                      const allSelected = funciones.every(
                        (f) => newAsientosPorFecha[f.id] && newAsientosPorFecha[f.id].length > 0,
                      );
    ...
    <div className="container mx-auto px-4 md:px-5 lg:px-8 2xl:px-20">
      ...
      <div className="flex items-center gap-x-3 my-4">
        {reservaExitosa ? (
          ...
          <Link
            href={`/eventos/${slug}?isAbono=true&abonoId=${currentAbonoId}`}
            className="bg-gray-200 hover:bg-gray-300 text-gray-800 px-3 py-2 rounded-md flex items-center gap-x-1"
          >
            <MdKeyboardBackspace className="text-4xl" />
          </Link>
        ...
        <h2 className="text-2xl lg:text-4xl text-gray-800 font-bold uppercase">
          ...
          <span className="font-bold">
            ...
            {funciones &&
            funciones.length > 0 &&
            currentFuncionIndex >= 0 &&
            funciones[currentFuncionIndex]?.fecha
      ...
      <div
        ref={pagoRef}
        className={`flex flex-col lg:flex-${reservaExitosa ? "row" : "col"} gap-3 mb-10`}
      >
        {/* Asientos o Inputs de Pago */}
        <div
          className={`w-${reservaExitosa ? "4/6" : "full"} bg-white shadow rounded-lg p-2 lg:p-4`}
        >
          ...
          {!reservaExitosa && <AsientosStatusComponent />}
        ...
        >
          {asientosSeleccionados.length > 0 ? (
            ...
            <div className="text-center text-gray-500">
              <figure>
                <img className="mx-auto" width={100} height={100} src="/caja_vacia.svg" alt="" />
              </figure>
      ...
      {compraExitosa && (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-50">
          <div
            className={`bg-neutral p-2 md:p-3 rounded-lg shadow-lg min-w-96 max-w-96 text-center`}
          >
            <div className="rounded-full w-12 h-12 bg-green-500 grid place-items-center mx-auto mb-5">
              <LuBadgeCheck className="text-neutral text-3xl" />
            </div>
            <h2 className="text-2xl font-bold text-gray-800">¡Compra exitosa!</h2>
            <hr className="my-3" />
            <p className="text-gray-600">
              Tus boletos para {evento?.nombre} {evento?.recinto.nombre} han sido comprados con
              éxito.
            </p>
            {/* <p className='text-gray-600 my-4'>Boleto enviado a: <span className='font-medium block'>Roberto@gmail.com</span></p> */}
            <p className="text-gray-600 my-4">
              Total pagado:{" "}
              <span className="block text-3xl font-bold text-gray-800">${calcularTotal()}</span>
            </p>
            <Link
              href="/perfil/mis_compras"
              onClick={() => clearAbonoBuilderState()}
              className="w-full block px-4 py-2 bg-accentBase hover:bg-emphasis mb-2 transition-colors text-neutral rounded-lg"
            >
              Ir a mis compras
            </Link>
            <button
              ...
              className="w-full px-4 py-2 mb-2 transition-colors text-gray-700 rounded-lg"
            >
              Continuar
            </button>
          </div>

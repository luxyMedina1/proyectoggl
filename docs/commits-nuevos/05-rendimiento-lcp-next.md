...
- 🔴 **La home no declara OG propio** (título/imagen específicos); hereda el fallback del layout. Al compartir `taquillavip.com/eventos` sale la tarjeta genérica. Añadir `openGraph`/`twitter` en el `generateMetadata` de la home (acción SEO de arriba), con imagen del banner destacado o el fallback de `opengraph-image.tsx`.

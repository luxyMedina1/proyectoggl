...
- 🟢 **Hecho.** El OG de fallback (`app/opengraph-image.tsx`) ya pinta el **logo de marca** (`config.logoMarca` → `config.logo` vía `getSiteConfig()`, extrayendo el PNG incrustado si el asset es SVG) sobre el degradado de marca, con fallback al texto (`nombreMarca` → `NEXT_PUBLIC_TITLE_APP`) solo si el backend no responde o el asset no trae raster.

- 🟢 **OG del sitio raíz — código OK.** `app/layout.tsx` (`generateMetadata`) ya usa `config.nombreMarca` (fallback `NEXT_PUBLIC_TITLE_APP`) en `title.default`, `og:title` y `og:site_name`, y la imagen la aporta `opengraph-image.tsx` (arriba). En el peor caso (backend caído + sin `NEXT_PUBLIC_TITLE_APP`) el título raíz cae a `"TaquillaVip"`, nunca a `"Eventos"` — ese `"Eventos"` de la captura era el literal de `/eventos`, ya corregido. **Lo que queda es de deploy, no de front:** en el túnel de Cloudflare hay que (a) setear `NEXT_PUBLIC_SITE_URL` al dominio del túnel y `NEXT_PUBLIC_TITLE_APP`, (b) confirmar que `config:sitio` de ese entorno devuelve `nombreMarca`/`logoMarca` reales, (c) validar con el debugger de FB/WhatsApp/Telegram contra la build de prod (el scraper cachea; puede necesitar re-scrape). `.env.example` ya documenta las dos variables.

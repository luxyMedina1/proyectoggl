...
const TransferirModal = ({
  ...
  return (
    <div
      className={`fixed inset-0 bg-black/50 ${overlayZIndexClass} flex items-center justify-center p-4`}
    >
            ...
            <>
              {items.length === 0 ? (
                <p className="text-gray-500 text-sm text-center py-8">
                  No tienes boletos transferibles. Es posible que ya estén en una transferencia
                  pendiente.
                </p>
              ) : (
                <>
                  <div className="flex items-center justify-between mb-3 px-2 py-2 bg-gray-50 rounded-lg">
                    ...
                    <button
                      ...
                      aria-pressed={seleccionados.size === items.length && items.length > 0}
                    ...
                    {items.map((i) => {
                      ...
                      return (
                        <li key={`${i.tipo ?? "item"}-${i.id}`}>
                          ...
                          >
                            <span
                              className={`inline-flex w-6 h-6 rounded-md border-2 flex-shrink-0 ${checked ? "bg-accentBase border-accentBase text-white" : "bg-white border-gray-300"} items-center justify-center`}
                            >
                              {checked && (
                                <svg viewBox="0 0 20 20" className="w-4 h-4 fill-current">
                                  <path d="M7.629 13.671 4.146 10.19l1.414-1.415 2.07 2.07 6.71-6.71 1.414 1.414z" />
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {i.detalle && (
                                <p className="text-xs text-gray-500 truncate">{i.detalle}</p>
                              )}
                            </div>
                            {i.precio !== undefined && (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500 truncate">
                              {a.amigo.email || a.amigo.telefono || ""}
                            </p>
                          </div>
                          <span
                            className={`w-5 h-5 rounded-full border-2 flex-shrink-0 ${checked ? "border-accentBase" : "border-gray-300"} flex items-center justify-center`}
                          >
          ...
          {paso === "exito" && (
            <div className="text-center py-6">
              ...
              <p className="text-xs text-gray-500 mt-3">
                Mientras está pendiente, el boleto seguirá en tu cuenta marcado como "transferencia
                pendiente".
              </p>
            </div>
          )}
          ...
          {paso === "seleccion" && (
            <>
              <button
                type="button"
                onClick={onClose}
                className="text-gray-600 font-medium px-4 py-2 hover:text-gray-800"
              >
                Cancelar
              </button>
              <button

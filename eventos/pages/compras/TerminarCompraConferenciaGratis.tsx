...
const TerminarCompraConferenciaGratis = () => {
  ...
  useEffect(() => {
    const confirmarPago = async () => {
      ...
      try {
        const res_cargo = await apiApplication.post(`/pagos/check/conferencia/cargo/gratis/${invitadoId}`, {
          reservaId,
        ...
        if (res_cargo?.status === 201 || res_cargo?.status === 200) {
          ...
          // 🔄 CORREO (registro interno; ya no se anuncia al usuario)
          await apiApplication.post(`/correos/sold-tickets-email-invitado-conferencia/${invitadoId}/${ticket_id}`);

          setIsVerifyMessage(res_cargo.data.message || 'Ocurrio un error contactar con soporte')
        ...
        <div className={`bg-white p-2 md:p-3 rounded-lg shadow-lg min-w-96 max-w-96 text-center min-h-60`}>
          {!compraExitosa ? (
            ...
            <>
              ...
              {infoCompra && (
                <div ref={qrRef} className='rounded-2xl min-h-96 mt-10 w-full md:max-w-lg mx-auto border shadow-sm bg-white'>
                  ...
                  <div className='p-2 md:p-4 space-y-5'>
                    ...
                    <p className='text-sm text-gray-700'>Tu registro para <span className="font-medium">{infoCompra.evento?.nombre}</span> ha sido confirmado. Presenta este código QR el día del evento para poder acceder al recinto.</p>
              ...
              <p className='text-gray-600 my-4'>Tus boletos ya están disponibles en <span className='font-semibold text-gray-800'>tu perfil</span> y en la <span className='font-semibold text-gray-800'>app móvil</span>.</p>
...
export default TerminarCompraConferenciaGratis;

    ...
    if (confirmacion.isConfirmed) {
      try {
        ...
        setTarjetas((prevTarjetas) =>
          prevTarjetas ? prevTarjetas.filter((tarjeta) => tarjeta.idtarjeta !== tarjetaId) : null,
        );
      } catch (error) {
        console.error(error);
        setCargando(false);
        Swal.fire({
          title: "Error",
          text: "Hubo un problema al eliminar la tarjeta.",
          icon: "error",
        });
      }
    }
  ...
  const handleGuardarTarjeta = async () => {
    ...
    if (isConfirmed) {
      try {
        ...
        setCargando(false);
        setTarjetas((prevTarjetas) =>
          prevTarjetas ? [...prevTarjetas, data.tarjeta] : [data.tarjeta],
        );
        ...
        <div className="grid grid-cols-7 gap-4 items-start mb-5">
          ...
          <section className="bg-gray-50 shadow-md rounded-xl col-span-7 md:col-span-4 lg:col-span-5 p-4">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <h2 className="text-gray-800 text-2xl 2xl:text-3xl font-semibold">
                Mis tarjetas guardadas
              </h2>
              <button
                onClick={() => setShowForm(true)}
                className="bg-gray-300 text-gray-700 hover:bg-gray-400 hover:text-gray-800 transition-colors rounded-md px-2 py-1 flex items-center"
              >
                <FaPlus className="w-5 flex-none" />
                Agregar tarjeta
              </button>
            </div>
            {showForm && (
              <fieldset className="mt-5 border border-gray-300 rounded-lg p-2 md:p-4 grid grid-cols-4 gap-4">
                <legend className="text-gray-600 text-xl font-medium col-span-2">
                  Información de la tarjeta
                </legend>
                <label
                  className="font-medium text-gray-700 col-span-4 md:col-span-2"
                  htmlFor="nombre"
                >
                  Nombre del titular
                  <input
                ...
                <label
                  className="font-medium text-gray-700 col-span-2 md:col-span-1"
                  htmlFor="codigo"
                >
                  Código de seguridad
                  <input
                ...
                <small className="col-span-4 text-gray-400 flex items-center gap-x-2">
                  <RiSecurePaymentLine className="text-lg" />
                  La información de tu tarjeta se guardará de forma segura.{" "}
                </small>
                <button
                  onClick={() => handleGuardarTarjeta()}
                  className="col-span-4 md:col-span-2 w-full bg-accentLight hover:bg-accentBase transition-colors px-2 py-2 rounded-lg text-neutral"
                >
                  Agregar
                </button>
                <button
                  onClick={() => setShowForm(false)}
                  className="col-span-4 md:col-span-2 w-full bg-gray-200 hover:bg-gray-300 transition-colors px-2 py-2 rounded-lg text-gray-700"
                >
                  Cancelar
                </button>
              </fieldset>
            ...
            {tarjetas?.length === 0 ? (
              ...
              tarjetas?.map((tarjeta) => (
                <div key={tarjeta.id} className="flex items-center gap-4 mb-5">
                  <div
                    className={`rounded-lg bg-gray-600 p-2 lg:p-4 text-white w-80 min-h-28 ${obtenerColorTarjeta(tarjeta.marca)}`}
                  >
                    ...
                    <p className="capitalize flex items-center justify-between">
                      {tarjeta.nombreEnTarjeta} <span>{tarjeta.tipo}</span>
                    </p>
                  </div>
                  <div>
                    <button

...
function MisComprasPage() {
  ...
  const [walletActive, setWalletActive] = useState<{ google: boolean; apple: boolean }>({
    google: false,
    apple: false,
  });
  ...
  useEffect(() => {
    const cargarWallet = async () => {
      const { data } = await apiApplication.get<{ google: boolean; apple: boolean }>(
        "/wallet/is-active",
      );
  ...
  const handleGetDetalleBoletos = async (eventoLista: Eventos) => {
    try {
      ...
      setEvento({
        ...response.evento,
        id: eventoLista.id,
        nombre: eventoLista.funcionNombre
          ? `${nombreBase} - ${eventoLista.funcionNombre}`
          : nombreBase,
  ...
  const renderEventos = (lista: Eventos[]) =>
    lista.map((e, index) => {
      ...
      return (
        <div
          key={index}
          className={`${vista === "grid" ? "w-full md:w-1/2 lg:w-1/3 p-2" : "w-full mb-4"}`}
        >
          <div
            className={`border border-gray-300 bg-white shadow-sm rounded-xl flex flex-col lg:flex-row gap-4 p-2 lg:p-4 ${vista === "grid" ? "lg:flex-col" : "flex-row"}`}
          >
            <img
            ...
            <div className={`${vista === "grid" ? "text-center" : "flex-1"}`}>
              <h3
                className={`text-gray-800 text-lg font-semibold flex flex-wrap items-center justify-between ${vista === "grid" ? "flex-col" : "flex-row mb-2"}`}
              >
                ...
                {mensajeDias !== "El evento ya pasó" && (
                  <Link
                    href={rutaEvento(
                      e,
                      e.funcionId
                        ? { id: e.funcionId, nombre: e.funcionNombre, fecha: e.fecha }
                        : null,
                    )}
                    className="text-accentLight font-normal flex items-center gap-2 mt-2 text-base"
                  >
              ...
              <div
                className={`flex flex-wrap ${vista === "grid" ? "justify-center" : "justify-between mb-2"}`}
              >
                <p
                  className={`text-gray-600 flex items-center gap-x-2 w-full ${vista === "grid" ? "flex-col mt-2" : "flex-row justify-between"}`}
                >
              ...
              <p
                className={`text-gray-600 flex items-center gap-x-2 ${vista === "grid" ? "flex-col" : "flex-row"}`}
              >
                ...
                <button
                  onClick={() => handleGetDetalleBoletos(e)}
                  className="text-accentLight ml-1"
                >
                  detalle de boleto(s)
                </button>
              </p>
  ...
  const todosVigentes = useMemo<BoletoVigente[]>(
    () => [...boletos, ...pasesGenerales],
    [boletos, pasesGenerales],
  );
  ...
  const boletosParaMulti: BoletoTransferible[] = useMemo(
    () =>
      todosVigentes
        ...
        .map((b) => {
          const esPaseGeneral = !!b.eventoSeccion;
          const cat = esPaseGeneral
            ? "Asiento General"
            : `Asiento ${b.categoria?.nombre ?? ""}`.trim();
  ...
  return (
    ...
    >
            ...
            {producto === "citypass" ? (
              ...
              <>
                ...
                <p className="text-gray-600 mb-8">
                  Aquí puedes consultar los boletos de los eventos que has comprado.
                </p>
                ...
                <div className="grid grid-cols-2 items-center gap-4 mb-5">
                  <div className="col-span-2 lg:col-span-1">
                    ...
                    >
                      Próximos Eventos{" "}
                      <span className="bg-[#f04343ff] rounded-full text-white w-10 inline-block ml-2">
              ...
              <>
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-gray-500 mt-1">
                      Aquí puedes consultar los boletos has comprado.
                    </p>
                  </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                        {boletosFiltrados.map((b) => (
                          <BoletoCard
                            ...
                            onDevolver={
                              b.esCompradorOriginal === false ? () => handleDevolver(b) : undefined
                            }
                ...
                {tab === "detalles" && (
                  <DetallesPedidoTab evento={evento} perfil={perfil} boletos={todosVigentes} />
                )}
              </>
            )}
          </section>
      ...
      <BoletoDetalleModal
        ...
        onGoogleWallet={() =>
          detalleBoleto && googlePass(detalleBoleto.quemadoUUID, !!detalleBoleto.eventoSeccion)
        }
        onAppleWallet={() =>
          detalleBoleto && applePass(detalleBoleto.quemadoUUID, !!detalleBoleto.eventoSeccion)
        }
      />
    </div>
  );
...
const FiltroChip = ({ active, label, count, onClick }: FiltroChipProps) => (
  ...
  >
    <span>{label}</span>
    <span
      className={`text-xs font-semibold rounded-full px-2 py-0.5 ${active ? "bg-white/20 text-white" : "bg-accentBase text-white"}`}
    >
      {count}
    </span>
  </button>

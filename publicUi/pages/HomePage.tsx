...
import { Link, useSearchParams } from '@/utils/nextRouterCompat';
...
export const HomePage = () => {
  ...
  // const [tipoSeleccionado, setTipoSeleccionado] = useState<"Comercial" | "Conferencia">("Comercial");
  ...
  const fetchEventos = useMemo(() => {
    return async () => {
      ...
      try {
        ...
        console.log("🚀 ~ HomePage ~ eventos:", eventos)

        if (Array.isArray(eventos) && eventos.length > 0) {

          setEventosOriginales(eventos);
          console.log(eventosOriginales)
          // const array = eventos.filter((element) => element.tipo === 'Comercial');
          setEventos(eventos);
  ...
  const iconosCategorias: { [key: string]: ReactElement } = {
  ...
  // const filtrarEventos = (tipo: "Comercial" | "Conferencia") => {
  //   setTipoSeleccionado(tipo);
  //   const filtrados = eventosOriginales.filter(e => e.tipo === tipo);
  //   setEventos(filtrados);
  //   setActiveEvent(filtrados[0] || null);
  // };

  // const saleStartTime = new Date("May 17, 2025 13:00:00").getTime();
  // const now = new Date().getTime();
  // const isSaleActive = now >= saleStartTime;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className='container mx-auto px-4 md:px-5 lg:px-8 2xl:px-20 inherit-mob'>
        <div className="w-full flex flex-col lg:flex-row h-80 md:h-80 mt-8 bg-emphasis rounded-2xl">
          <div className="w-full lg:w-1/2 flex flex-col justify-center p-4 lg:p-8 min-h-0">
            ...
            {eventos.length > 0 ? (
              <Link
                to={activeEvent ? rutaEvento(activeEvent) : "/eventos"}
                onClick={(e) => {
      ...
      <section className="container mx-auto px-4 md:px-5 lg:px-8 2xl:px-20 mb-5 mt-5">
        ...
        {/* 🔘 Selector premium centrado y responsivo */}
        {/* Comentado porque ya no les gusto asi */}
        {/* {config?.habilitarConferencias && (
          <div className="flex flex-col items-center">
            <div className="relative flex w-72 bg-gray-100 dark:bg-gray-800 rounded-full p-1 mb-8 shadow-md border border-gray-300 dark:border-gray-700">
              <div
                className={`absolute top-1 bottom-1 w-1/2 left-0 rounded-full transition-all duration-500 ease-in-out ${tipoSeleccionado === "Conferencia"
                    ? "translate-x-full bg-gradient-to-r from-purple-500 to-indigo-600"
                    : "bg-gradient-to-r from-blue-500 to-cyan-500"
                  } shadow-lg`}
              ></div>

              <button
                className={`relative flex-1 text-center font-semibold z-10 transition-colors duration-300 ${tipoSeleccionado === "Comercial" ? "text-white" : "text-gray-500 dark:text-gray-300"
                  }`}
                onClick={() => filtrarEventos("Comercial")}
              >
                Eventos
              </button>
              <button
                className={`relative flex-1 text-center font-semibold z-10 transition-colors duration-300 ${tipoSeleccionado === "Conferencia" ? "text-white" : "text-gray-500 dark:text-gray-300"
                  }`}
                onClick={() => filtrarEventos("Conferencia")}
              >
                Conferencias
              </button>
            </div>
                  ...
                  {eventos.map((evento) => (
                    ...
                    >
                      ...
                      <figure className="relative bg-emphasis">
                        <Link to={rutaEventoPorBase(evento, config?.rutaBase)} onClick={(e) => handleVerClick(e, evento)}>
                          <img
                        ...
                        {config?.mostrarOjo && (
                          <Link
                            to={rutaEventoInformacion(evento)}
                      ...
                      <div className="px-3 mbs-auto">
                        <Link
                          ...
                          to={rutaEventoPorBase(evento, config?.rutaBase)}
                    ...
                    {abonosDisponibles.map((abono) => {
                      ...
                      return (
                        <div className="flex flex-col shadow-md rounded-xl bg-white border border-slate-200 p-2 md:p-3">
                          ...
                          <Link
                            key={`abono-${abono.id}`}
                            to={`${rutaEvento(abono.evento)}?isAbono=true&abonoId=${abono.id}`}
              ...
              <>
                ...
                <div className="max-h-60 overflow-y-auto flex flex-col gap-3 pr-2 scrollbar-thin scrollbar-thumb-gray-300">
                  {eventoSeleccionado.funciones?.map((funcion) => {
                    return <Link
                      key={funcion.id}
                      to={rutaEventoPorBase(eventoSeleccionado, configPorTipo[eventoSeleccionado.tipo as keyof typeof configPorTipo]?.rutaBase, funcion)}
